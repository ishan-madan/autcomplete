...
public class BinarySearchAutocomplete implements Autocompletor {
	...
	public List<Term> topMatches(String prefix, int k) {
		...
		PriorityQueue<Term> deezNuts = new PriorityQueue<>(Comparator.comparing(Term::getWeight));
		...
		for (int i = first; i <= last; i++) {
			...
			if (deezNuts.size() < k) {
				deezNuts.add(myTerms[i]);
			} else if (deezNuts.peek().getWeight() < myTerms[i].getWeight()) {
				deezNuts.remove();
				deezNuts.add(myTerms[i]);
			}
		}

		int numResults = Math.min(k, deezNuts.size());
		LinkedList<Term> ret = new LinkedList<>();
		for (int i = 0; i < numResults; i++) {
			ret.addFirst(deezNuts.remove());
		}
		return ret;
